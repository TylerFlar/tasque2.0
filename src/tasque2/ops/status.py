...
def get_system_status(session: Session) -> SystemStatus:
    return SystemStatus(
        ...
        or 0,
        schedules_enabled=session.scalar(select(func.count()).select_from(Schedule).where(Schedule.enabled.is_(True)))
        or 0,
        workflow_runs=_count_by(session, WorkflowRun.status),
        ready_work=session.scalar(select(func.count()).select_from(WorkItem).where(WorkItem.status == "ready")) or 0,
        running_work=session.scalar(select(func.count()).select_from(WorkItem).where(WorkItem.status == "running"))
        or 0,
    )
...
def work_status_counts(session: Session) -> dict[str, int]:
    return _count_by(session, WorkItem.status)
