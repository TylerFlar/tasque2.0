...
class ReportService:
    ...
    def work_report(self, work_item_id: str) -> Report:
        ...
        attempts = self.session.scalars(
            select(WorkAttempt).where(WorkAttempt.work_item_id == work.id).order_by(WorkAttempt.attempt_number)
        ).all()
        ...
        for attempt in attempts:
            provider_runs.extend(
                self.session.scalars(select(ProviderRun).where(ProviderRun.attempt_id == attempt.id)).all()
            )
    ...
    def workflow_report(self, workflow_run_id: str) -> Report:
        ...
        work_items = self.session.scalars(
            select(WorkItem).where(WorkItem.workflow_run_id == run.id).order_by(WorkItem.created_at)
        ).all()
    ...
    def _render_work_report(self, data: dict[str, Any]) -> str:
        ...
        runs_by_attempt = {run["attempt_id"]: run for run in data["provider_runs"]}
        for attempt in data["attempts"]:
            ...
            run = runs_by_attempt.get(attempt["id"]) or {}
            usage = run.get("usage") or {}
            if usage.get("input_tokens") is not None:
                lines.append(
                    "  usage: "
                    f"input={usage.get('input_tokens', 0)} cache_read={usage.get('cache_read_tokens', 0)} "
                    f"cache_write={usage.get('cache_write_tokens', 0)} output={usage.get('output_tokens', 0)} "
                    f"messages={usage.get('messages', 0)} est_cost=${usage.get('estimated_cost_usd', 0)}"
                )
            if run.get("stream_artifact_id"):
                lines.append(f"  stream artifact: {run['stream_artifact_id']}")
        lines.extend(["", "## Events"])
    ...
    def _provider_run_data(self, provider_run: ProviderRun) -> dict[str, Any]:
        return {
            "id": provider_run.id,
            "attempt_id": provider_run.attempt_id,
            ...
            "stream_artifact_id": provider_run.stdout_artifact_id,
