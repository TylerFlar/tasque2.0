...
from tasque2.daemon import reset_artifact_retention_clock, reset_background_pool
...
def fresh_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    reset_engine()
    create_schema()
    # The limit gate, the retention clock and the background work pool are all
    ...
    reset_artifact_retention_clock()
    yield db_path
    reset_background_pool()
    reset_engine()
    reset_provider_limit_gate()
